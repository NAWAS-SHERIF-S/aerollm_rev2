...
import { Plane, Wrench, ShieldCheck, Clock, X, ChevronRight, RefreshCw, AlertTriangle } from 'lucide-react';
    ...
    <div className="max-w-7xl mx-auto px-4 lg:px-8 py-8 space-y-8">
          ...
          {['ALL', 'ACTIVE', 'MAINTENANCE', 'INSPECTION'].map((tab) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                filter === tab ? 'bg-cyan-500 text-black shadow' : 'text-slate-400 hover:text-white'
              }`}
            >
              {tab}
            </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredFleet.map((ac) => (
          <div
            ...
            className="glass-panel p-6 space-y-4 cursor-pointer hover:border-cyan-500/40 transition-all group"
          >
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-2xl font-extrabold text-white font-['Outfit'] group-hover:text-cyan-400 transition-colors">
                    {ac.id}
                  </h3>
                  <p className="text-xs text-slate-400 font-mono">Model: {ac.model}</p>
                </div>
              </div>
      ...
      {selectedAircraft && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
          <div className="w-full max-w-3xl glass-panel p-8 space-y-6 relative border-cyan-500/40 max-h-[90vh] overflow-y-auto">
            
            <button
              onClick={() => setSelectedAircraft(null)}
              className="absolute top-6 right-6 p-2 rounded-lg bg-white/5 hover:bg-white/10 text-slate-400 hover:text-white"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Aircraft Overview</span>
                <h2 className="text-3xl font-extrabold text-white font-['Outfit']">{selectedAircraft.id}</h2>
                <p className="text-xs text-cyan-400 font-mono">Model: {selectedAircraft.model} • Status: {selectedAircraft.status}</p>
              </div>
            </div>
            ...
            <div className="space-y-3 pt-4 border-t border-white/10">
              ...
              {aircraftHistory.length === 0 ? (
                <p className="text-sm text-slate-400 py-4 text-center">No maintenance logs recorded for {selectedAircraft.id} yet.</p>
              ) : (
                <div className="space-y-3">
                  {aircraftHistory.map((rec) => (
                    <div key={rec.id} className="p-4 rounded-xl bg-white/5 border border-white/10 space-y-2">
                      <div className="flex items-center justify-between text-xs text-slate-400">
                        <span className="font-mono text-cyan-400 font-bold">Log #{rec.id}</span>
