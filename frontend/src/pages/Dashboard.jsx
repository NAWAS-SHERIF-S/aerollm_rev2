      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7 glass-panel p-6 space-y-6">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-slate-400 font-mono">GET /api/dashboard/stats</span>
          </div>
              ...
              return (
                <div key={act} className="space-y-1.5">
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className="font-mono text-cyan-400 font-bold">{count} Action{count > 1 ? 's' : ''} ({pct}%)</span>
                  </div>
        ...
        <div className="lg:col-span-5 glass-panel p-6 space-y-6">
          ...
          <div className="space-y-3">
            <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-between">
              <div>
                <p className="text-xs text-slate-400">Active Flight Status</p>
                <p className="text-lg font-bold text-emerald-400 font-['Outfit']">{stats?.active_aircraft ?? 3} Aircraft (60%)</p>
              </div>
            ...
            <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/30 flex items-center justify-between">
              <div>
                <p className="text-xs text-slate-400">In Maintenance Bay</p>
                <p className="text-lg font-bold text-amber-400 font-['Outfit']">{stats?.aircraft_in_maintenance ?? 1} Aircraft (20%)</p>
              </div>
      ...
      <div className="glass-panel p-6 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="text-xs text-cyan-400 hover:text-white font-semibold flex items-center gap-1"
          >
                  ...
                  <td>
                    <div className="flex flex-wrap gap-1">
                      {item.actions?.map((a, idx) => (
                        <span key={idx} className="text-[10px] font-bold px-2 py-0.5 rounded bg-cyan-500/10 text-cyan-400 border border-cyan-500/20">
                          {a}
                        </span>
                      ))}
                    </div>
