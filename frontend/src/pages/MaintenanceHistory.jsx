...
import { Search, Wrench, Filter, Calendar, ChevronLeft, ChevronRight, Eye, RefreshCw, X } from 'lucide-react';
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 lg:px-8 py-8 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/10 pb-6">
        ...
        <button
          onClick={loadHistory}
          className="btn-secondary-glass text-xs py-2 px-4 self-start sm:self-auto flex items-center gap-1.5"
        >
      ...
      <div className="glass-panel p-4 flex flex-col md:flex-row gap-4 items-center justify-between">
        ...
        {/* Search Input */}
        <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 w-full md:w-96 focus-within:border-cyan-400 transition-all">
          ...
          <input
            ...
            className="bg-transparent border-none outline-none text-white text-sm placeholder-slate-500 w-full font-sans"
          />
          {searchQuery && (
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          <span className="text-xs text-slate-400 font-semibold flex items-center gap-1">
            <Filter className="w-3.5 h-3.5 text-cyan-400" /> Aircraft:
          </span>
          <select
          ...
          <table className="custom-table">
            ...
            <tbody>
              ...
              {filteredHistory.map((item) => (
                <tr key={item.id}>
                  <td className="font-mono text-xs text-cyan-400 font-bold">#{item.id}</td>
                  <td>
                  ...
                  <td>
                    <button
                      ...
                      className="p-2 rounded-lg bg-white/5 hover:bg-cyan-500/20 text-cyan-400 border border-white/10 hover:border-cyan-500/40 transition-all flex items-center gap-1 text-xs"
                    >
      ...
      {selectedRecord && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
          <div className="w-full max-w-2xl glass-panel p-8 space-y-6 relative border-cyan-500/40">
            <button
              onClick={() => setSelectedRecord(null)}
              className="absolute top-6 right-6 p-2 rounded-lg bg-white/5 text-slate-400 hover:text-white"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-400 font-mono">{new Date(selectedRecord.created_at).toUTCString()}</p>
            </div>

            <div className="p-4 rounded-xl bg-[#070b19] border border-white/10 space-y-2">
              <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Original Technician Report</p>
              <p className="text-sm text-slate-200 leading-relaxed font-sans">{selectedRecord.report}</p>
            </div>

            <div className="space-y-2">
              <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">NovaTRix Extracted Actions</p>
              <div className="flex flex-wrap gap-2">
                {selectedRecord.actions?.map((act, idx) => (
                  <span key={idx} className="px-3 py-1.5 rounded-lg bg-cyan-500/20 text-cyan-400 font-bold text-xs border border-cyan-500/30">
                    ✓ {act}
                  </span>
                ))}
              </div>
            ...
            <div className="pt-4 border-t border-white/10 flex justify-end">
              <button onClick={() => setSelectedRecord(null)} className="btn-secondary-glass text-xs py-2 px-5">
                Close View
              </button>
            </div>
          </div>
